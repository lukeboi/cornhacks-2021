...
const handler = ({
  ...
  (0, _execa.default)(`yarn msw init "${staticAssetsFolder}"`, undefined, {
    stdio: 'inherit',
    shell: true,
    cwd
  });
  (0, _execa.default)(`yarn ${build ? 'build' : 'start'}-storybook`, (0, _filter.default)(_context = ['--config-dir ../node_modules/@redwoodjs/core/config/storybook', !build && `--port ${port}`, !build && '--no-version-updates', `--static-dir "${staticAssetsFolder}"`, open && '--ci']).call(_context, Boolean), {
    stdio: 'inherit',
    shell: true,
    cwd
  });

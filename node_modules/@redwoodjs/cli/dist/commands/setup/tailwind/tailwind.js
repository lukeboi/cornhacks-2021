...
const handler = async ({
  ...
  const tasks = new _listr.default([{
    ...
    task: async () => {
       ...
       */
      const configExists = _fs.default.existsSync(_path.default.join((0, _lib.getPaths)().web.base, 'tailwind.config.js'));

      if (!force && configExists) {
        throw new Error('Tailwindcss config already exists.\nUse --force to override existing config.');
      } else {
        await (0, _execa.default)('yarn', ['tailwindcss', 'init']); // opt-in to upcoming changes

        const config = _fs.default.readFileSync('tailwind.config.js', 'utf-8');

        const uncommentFlags = str => str.replace(/\/{2} ([\w-]+: true)/g, '$1');

        const newConfig = config.replace(/future.*purge/s, uncommentFlags);

        _fs.default.writeFileSync('tailwind.config.js', newConfig);
        /**
        ...
        _fs.default.renameSync('tailwind.config.js', 'web/tailwind.config.js');
      }
    }
  }, {

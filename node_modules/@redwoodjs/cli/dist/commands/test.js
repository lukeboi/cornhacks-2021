...
const handler = async ({
  ...
  const args = (0, _filter.default)(_context3 = ['--passWithNoTests', collectCoverage && '--collectCoverage']).call(_context3, Boolean); // If the user wants to watch, set the proper watch flag based on what kind of repo this is
  ...
  if (!sides.length) {
    ...
    (0, _forEach.default)(_context4 = (0, _structure.getProject)().sides).call(_context4, side => sides.push(side));
  }
  ...
  try {
    ...
    const DATABASE_URL = process.env.TEST_DATABASE_URL || cacheDirDb; // Create a test database

...
const rwPackages = ['@redwoodjs/core', '@redwoodjs/api', '@redwoodjs/web', '@redwoodjs/router', '@redwoodjs/auth', '@redwoodjs/forms'].join(' '); // yarn upgrade-interactive does not allow --tags, so we resort to this mess
...
const installPr = (pr, isAuth) => {
  ...
  const mainString = `yarn add -DW ${packageUrl('core')} ${packageUrl('cli')} ` + `&& yarn workspace api add ${packageUrl('api')} ` + `&& yarn workspace web add ${packageUrl('web')} ` + `${packageUrl('router')} ${packageUrl('forms')}`;

...
const files = async ({
  ...
  return { ...(await (0, _sdl.files)({ ...(0, _lib.getDefaultArgs)(_sdl.builder),
      name,
      crud: true,
      typescript,
      javascript
...
const builder = yargs => {
  ...
  }).epilogue(`Also see the ${(0, _terminalLink.default)('Redwood CLI Reference', 'https://redwoodjs.com/reference/command-line-interface#generate-scaffold')}`);
...
const handler = async ({
  ...
  const schema = await (0, _lib.getSchema)((0, _pascalcase.default)(_pluralize.default.singular(model)));
  const line1 = _chalk.default.bold.yellow('WARNING') + `: Because the data model "${(0, _pascalcase.default)(model)}" contains a`;
  ...
  const line4 = _chalk.default.underline.blue('https://redwoodjs.com/docs/schema-relations');

  try {
    await t.run();

    if ((0, _helpers.relationsForModel)(schema).length) {
      console.log((0, _boxen.default)(line1 + '\n' + line2 + '\n' + line3 + '\n' + line4, {
        padding: 1,
        margin: 1,
        borderStyle: 'single'
      }));

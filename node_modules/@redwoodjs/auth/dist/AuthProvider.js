...
class AuthProvider extends _react.default.Component {
  constructor(props) {
    ...
    this.getCurrentUser = async () => {
      const response = await window.fetch("".concat(window.__REDWOOD__API_PROXY_PATH, "/graphql"), {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
          'auth-provider': this.rwClient.type,
          authorization: "Bearer ".concat(this.state.authToken)
        },
    ...
    this.hasRole = role => {
      if (typeof role !== 'undefined' && this.state.currentUser && this.state.currentUser.roles) {
        ...
        if ((0, _isArray.default)(role)) {
          ...
          return (0, _some.default)(_context2 = this.state.currentUser.roles).call(_context2, r => (0, _includes.default)(role).call(role, r));
        }
      }

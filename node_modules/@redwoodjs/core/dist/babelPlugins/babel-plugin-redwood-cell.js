    ...
    visitor: {
      ...
      Program: {
        exit(path) {
          ...
          path.node.body.unshift(t.importDeclaration([t.importSpecifier(t.identifier('withCell'), t.identifier('withCell'))], t.stringLiteral('@redwoodjs/web'))); // Insert at the bottom of the file:
          // + export default withCell({ QUERY?, Loading?, Succes?, Failure?, Empty?, beforeQuery?, afterQuery? })

          path.node.body.push(t.exportDefaultDeclaration(t.callExpression(t.identifier('withCell'), [t.objectExpression((0, _map.default)(exportNames).call(exportNames, name => t.objectProperty(t.identifier(name), t.identifier(name), false, true)))])));

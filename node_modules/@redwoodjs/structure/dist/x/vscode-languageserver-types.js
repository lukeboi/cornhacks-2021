...
function LocationLike_toLocation(x) {
  ...
  if (typeof x === 'object') {
    ...
    if (_vscodeLanguageserverTypes.Location.is(x)) return x;
    if (ExtendedDiagnostic_is(x)) return {
      uri: x.uri,
...
function ExtendedDiagnostic_is(x) {
  ...
  if (!_vscodeLanguageserverTypes.Diagnostic.is(x.diagnostic)) return false;
  return true;
}
...
function ExtendedDiagnostic_format(d, opts) {
  ...
  let base = 'file://';
  if (cwd) base = (0, _URL.URL_file)(cwd);
  if (!(0, _endsWith.default)(base).call(base, '/')) base += '/';
...
function FileSet_fromTextDocuments(documents) {
  ...
  for (const uri of (0, _keys2.default)(documents).call(documents)) files[uri] = documents.get(uri).getText();
...
function WorkspaceEdit_fromFileSet(files, getExistingFileText) {
  ...
  for (const uri of (0, _keys.default)(files)) {
...
function Range_full(text, cr = '\n') {
  if (text === '') return _vscodeLanguageserverTypes.Range.create(0, 0, 0, 0);
  const lines = text.split(cr);
  if (lines.length === 0) return _vscodeLanguageserverTypes.Range.create(0, 0, 0, 0);

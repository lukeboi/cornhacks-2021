...
function URL_toFile(uriOrFilePath, sep = _path.sep) {
  ...
  if ((0, _startsWith.default)(uriOrFilePath).call(uriOrFilePath, FILE_SCHEME)) return fileUriToPath(uriOrFilePath, sep);
  const p = (0, _path.normalize)(uriOrFilePath);
  if (!(0, _path.isAbsolute)(p)) throw new Error('absolute path expected: ' + p);
  return p;
}
...
function fileUriToPath(uri, sep = _path.sep) {
  ...
  if (host) host = sep + sep + host; // 3.2  Drives, drive letters, mount points, file system root
  ...
  if (!(0, _includes.default)(_context = parts[0]).call(_context, ':')) parts.unshift('');

...
function process_env_findAll(dir) {
  return (0, _Array.iter)(function* () {
    for (const file of _glob.default.sync((0, _path.join)(dir, 'src/**/*.{js,ts,jsx,tsx}'))) yield* process_env_findInFile(file, (0, _fsExtra.readFileSync)(file).toString());
  });
}

function process_env_findInFile(filePath, text) {
  if (!(0, _includes.default)(text).call(text, 'process.env')) return [];

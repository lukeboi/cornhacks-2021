...
let RWPage = (_dec = (0, _decorators.lazy)(), _dec2 = (0, _decorators.lazy)(), _dec3 = (0, _decorators.lazy)(), _dec4 = (0, _decorators.lazy)(), (_class = class RWPage extends _ide.FileNode {
  ...
  get filePath() {
    const f = (0, _path2.directoryNameResolver)(this.path);
    if (!f) throw new Error(`could not resolve index filename for directory '${this.path}' using dirname convention`);
    return f;
  }
  ...
  get layoutName() {
    ...
    const candidates = (0, _map2.default)(_context2 = this.parent.layouts).call(_context2, l => l.basenameNoExt);
    if (candidates.length === 0) return undefined;

    for (const tag of this.sf.getDescendantsOfKind(tsm.SyntaxKind.JsxOpeningElement)) {
      const t = tag.getTagNameNode().getText(); //?

      if ((0, _includes.default)(candidates).call(candidates, t)) return t;
    }
  ...
  get actionRemove() {
    const edits = new _map.default(); // delete directory (MyPage/...)

    edits.set((0, _path.dirname)(this.filePath), undefined); // removing a page also removes its route

    if (this.route) edits.set(this.route.jsxNode, undefined); // TODO: we need to transform this edits map to a standard edits map (with locations)
